...
def _paginate(
    items: list[_FakeArticle], page: int, size: int
) -> tuple[list[_FakeArticle], int, int]:
    """Pure Python mirror of the routes.py pagination logic."""
    total = len(items)
    offset = (page - 1) * size
...
# Feature: article-classifier, Property 8: API pagination correctness
@given(
    articles=st.lists(_article_strategy(), max_size=50),
    page=st.integers(min_value=1, max_value=10),
    size=st.integers(min_value=1, max_value=100),
)
@h_settings(max_examples=100)
def test_api_pagination_correctness(articles, page, size):
    """Property 8: For any valid pagination params, response contains at most `size`
    items, total equals full count, and pages equals ceil(total/size).
    Validates: Requirements 6.9"""
    page_items, total, pages = _paginate(articles, page=page, size=size)

    assert len(page_items) <= size, (
        f"Page returned {len(page_items)} items but size={size}"
    )

    assert total == len(articles), (
        f"total={total} does not match full item count={len(articles)}"
    )
    ...
    offset = (page - 1) * size
    expected_slice = articles[offset : offset + size]
    assert page_items == expected_slice, (
        f"Page slice mismatch: got {[a.id for a in page_items]}, "
        f"expected {[a.id for a in expected_slice]}"
    )
